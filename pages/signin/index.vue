			...
			.col.login__form.d-flex.flex-column.justify-content-around.px-5.mx-5
				...
				.login__body.p-5
					form(@submit.prevent="signinUser")
						...
						.form-group.login__form__group
							input(type="password" id="password" class="form-control login__input" placeholder=" " v-model="userData.password")/
							span(class="login__label") Entrez votre mot de passe

						.form-group.form-check
							input(type="checkbox" class="form-check-input" id="remember" v-model="userData.rememberMe")/
							label(class="form-check-label font-weight-bold text-muted" form="remember") Se Souvenir de moi

						.row.mx-0.mt-3.align-items-center.justify-content-between
							button(type="submit" class="btn btn-secondary font-weight-bold" :disabled="!userData.email || !userData.password") Se Connecter
								span(class="spinner-border spinner-border-sm ml-2 mb-1" v-if="onSignin")
							nuxt-link(:to="localePath('password-reset')" class="link link-item text-muted") {{ $t('signIn_password_forgotten') }}
